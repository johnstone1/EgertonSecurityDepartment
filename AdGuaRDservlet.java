...
public class AdGuaRDservlet extends HttpServlet {
 ...
 LocalDate two=LocalDate.now();
	...
	if(date==""||date.equals(null)||ssn==""||ssn.equals(null)||name==""||name.equals(null)||phoneno==""||phoneno.equals(null)) {
		...
		request.setAttribute("Message","Enter today's or  coming days :REGISTRATION FAILED");
		getServletContext().getRequestDispatcher("/OperationalDashboard.jsp").forward(request,response);
		
		
	}
	else if((phoneno.length()!=10) ||(phoneno.charAt(0)!='0' || phoneno.charAt(1)!='7')) {
		System.out.println(phoneno.charAt(0));
		System.out.println(phoneno.charAt(1));

		request.setAttribute("Message","Enter valid phone number:REGISTRATION FAILED");
		getServletContext().getRequestDispatcher("/OperationalDashboard.jsp").forward(request,response);
	}
	else {
